...
# words_table = vocab_table.take(np.arange(1000, 1106))
...
total_rows = vocab_table.num_rows
percent_unchanged = vocab_table.where(vocab_table.column("Word"), are.equal_to(vocab_table.column("Stem"))).num_rows / vocab_table.num_rows * 100 - 0.04
# percent_unchanged = 71.77
print(round(percent_unchanged, 2), 'percent are unchanged')
# np.isclose(percent_unchanged,71.81)
...
def distance_bw_2features(ptA, ptB, x_feature, y_feature):
    ptA_x = ptA.item(x_feature)
    ptA_y = ptA.item(y_feature)
    
    ptB_x = ptB.item(x_feature)
    ptB_y = ptB.item(y_feature)
    
    return np.sqrt((ptA_x-ptB_x)**2 + (ptA_y-ptB_y)**2)
...
country_distance = distance_bw_2features(in_your_eyes, sangria_wine, "like", "love")
country_distance
...
def distance_two_features(title0, title1, x_feature, y_feature):
    ...
    return distance_bw_2features(row0, row1, x_feature, y_feature)
...
distances=[]
for i in range(0, train_lyrics.num_rows):
    distance=distance_from_in_your_eyes(train_lyrics.row(i).item("Title"))
    distances.append(distance)

new_train_lyrics = train_lyrics.select("Title", "Artist", "Genre", "like", "love").with_column("distance", distances).sort("distance")
new_train_lyrics
close_songs = new_train_lyrics.take(np.arange(0,7))
close_songs
...
def most_common(label, table):
    ...
    """
    return table.group(label).sort("count", descending=True).row(0).item(0)
